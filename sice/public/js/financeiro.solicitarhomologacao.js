$(".icoVisualizar").click(function() {
	var titulo = 'Detalhes do Bolsista';
	$.ajax({
		url : $(this).attr('href'),
...
$(".icoReceber").click(function() {
	$.ajax({
		...
		success : function(data) {
			...
			var titulo = 'Motivo da Devolu&ccedil;&atilde;o';
	...
	$.ajax({
        ...
        success: function(html){
        	...
        	if($(html).find("#mensagens .msgSucesso p").html() == 'FECHAR_POPUP'){
        		$.alerts._hide();
        		window.location.href = baseUrl + '/index.php/financeiro/solicitarhomologacao/form/NU_SEQ_USUARIO/'+ $(html).find("#NU_SEQ_USUARIO").val();
        		return false;
        	}
...
window.onload = function(){
	...
	th.html($('<input />').attr('type','checkbox').bind('click',function(){
	               ...
	               $(this).parent().unbind();
	               $(dataTablesObj).find('tbody input[type=checkbox]').each(function(){
		               	if($(this).attr('disabled') != 'disabled'){
		            		$(this).attr('checked',(val == "checked" || val == true ? "checked" : false));
		              	}
	               });
	}));  
};