 ...
 * @version    $Id: Table.php 232 2011-03-04 13:54:58Z TheoziranSilva $
 * @author     Bento Vilas Boas <geral@petala-azul.com >
 * 
 * 
 * @author  Jânio Eduardo Vasconcellos de Magalhães <janio.magalhaes@fnde.gov.br >
 * @version $Id: Table.php 232 2011-03-04 13:54:58Z TheoziranSilva $
 */
class Fnde_Grid_Table extends Bvb_Grid implements Bvb_Grid_Deploy_Interface {
    const OUTPUT = 'table';
    const MESSAGE_NO_RECORD = 'No records found';

    protected static $_message = array(
        self::MESSAGE_NO_RECORD => Fnde_Grid::DEFAULT_MESSAGE_NO_RECORD,
    );
    /**
    ...
    function __construct($options) {
        ...
        $this->addTemplateDir('Fnde/Grid/Template', 'Fnde_Grid_Template', 'table');
    ...
    protected function _pagination() {
        ...
        if ($actual != $npaginas) {
            if ($this->getInfo("ajax") !== false) {
                ...
                //está entrando aqui para paginação
            ...
            $script .= "function _" . $this->getGridId() . "gridChangeFilters(fields,url,Ajax)
{
    ...
    //Adicionado novos combos na action no padrão FNDE
    public function getCombo() {
        ...
        if ($this->_options['acao']) {
            ...
            if ($this->_options['acao']['evento']) {
                $stHtmlCombo .='<input type="button" onclick="javascript:' . $this->_options["acao"]["evento"] . '();" value="OK" />';
            } else {
                $stHtmlCombo .='<input type="button" onclick="if($(\'select#acao\').val() != \'\'){$(\'#formAction-' . $hash . '\').val($(\'select#acao\').val());$(\'#formGrid-' . $hash . '\').submit();}" value="OK" />';
            }
        }
