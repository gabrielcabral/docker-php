 ...
 * @package Fnde_View
 * @subpackage Helper
 * @author Alberto Viana <alberto.viana@fnde.gov.br>
 * @version 1.3
 */
class Fnde_View_Helper_DataTables
{
    ...
    private function buildBody()
    {
        ...
        for ($i = 0; $i < $total; $i++) {
            ...
            if ($this->hasInput()) {
                $body .= '<td class="' . self::CLASS_INPUT . '">' . $this->buildInput($this->_id,
                                $this->_data[$i][$this->_id]) . '</td>';
            }

            $links = '';
            if (!is_null($this->_rowAction)) {

                $links = (isset($this->_rowAction[$i]) ?
                                $this->buildRowAction($this->_rowAction[$i], $this->_data[$i]) :
    ...
    {
        return "<input type=\"{$this->_rowInput}\" name=\"{$name}\" value=\"$value\" />";
    }
    ...
    private function buildMainAction()
    {
        ...
        if (count($this->_mainAction) > 0) {
            $mainAction .= '<div class="listagemAcoes">' . PHP_EOL
                    . '<label>' . PHP_EOL
                    . '<span>+</span> ações: ' . PHP_EOL
                    . '<select name="table_main_action" id="table_main_action">' . PHP_EOL
                    . '<option value="">Selecione...</option>' . PHP_EOL;
            foreach ($this->_mainAction as $key => $value) {
                $mainAction .= "<option value='{$value}'>{$key}</option>";
            }
            $mainAction .= '</select>' . PHP_EOL
                    . '</label>' . PHP_EOL
    ...
    **/
    ...
    public function __toString()
    {
        ...
        if($this->_autoCallJs)
            ...
            $html .= "<script type='text/javascript'>jQuery(function(){ oFnde.dataTable('#{$this->_tableAttribs['id']}','{$headerActive}','{$footerActive}',{$JsSettings}); } );</script>";
